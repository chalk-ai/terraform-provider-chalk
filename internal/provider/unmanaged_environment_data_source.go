...
var _ datasource.DataSource = &UnmanagedEnvironmentDataSource{}

func NewUnmanagedEnvironmentDataSource() datasource.DataSource {
	return &UnmanagedEnvironmentDataSource{}
}

type UnmanagedEnvironmentDataSource struct {
	client *client.Manager
}

// UnmanagedEnvironmentDataSourceModel mirrors ManagedEnvironmentDataSourceModel
// plus kube_service_account_name and kube_cluster_mode (the two unmanaged-specific
// fields), and like its managed sibling deliberately omits online_store_secret.
type UnmanagedEnvironmentDataSourceModel struct {
...
func (d *UnmanagedEnvironmentDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_unmanaged_environment"
}

func (d *UnmanagedEnvironmentDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Reads a Chalk unmanaged (BYOC) environment by ID. Companion to `chalk_managed_environment` for environments hosted on a customer-managed Kubernetes cluster. Deliberately omits `online_store_secret`.",
		Attributes: map[string]schema.Attribute{
			...
			"kube_service_account_name":   schema.StringAttribute{Computed: true},
			"kube_cluster_mode":           schema.StringAttribute{Computed: true},
...
func (d *UnmanagedEnvironmentDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *UnmanagedEnvironmentDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data UnmanagedEnvironmentDataSourceModel
	resp.Diagnostics.Append(req.Config.Get(ctx, &data)...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Trace(ctx, "read chalk_unmanaged_environment data source", map[string]any{"id": data.Id.ValueString()})

	tc := d.client.NewTeamClient(ctx, data.Id.ValueString())
	envResp, err := tc.GetEnv(ctx, connect.NewRequest(&serverv1.GetEnvRequest{}))
	if err != nil {
		resp.Diagnostics.AddError(
			"Error Reading Unmanaged Environment",
			fmt.Sprintf("Could not read unmanaged environment %s: %v", data.Id.ValueString(), err),
		)
		return
	}
