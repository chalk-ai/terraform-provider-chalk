...
- `uri` (String) URI of the existing bucket (and optional path prefix), e.g. `s3://bucket/prefix`, `gs://bucket/prefix`, `https://<account>.blob.core.windows.net/<container>[/path]`, `abfss://<container>@<account>.dfs.core.windows.net[/path]`, or `mock://bucket`. When `kind` is set, the scheme must match it. Changing this forces a new resource.
